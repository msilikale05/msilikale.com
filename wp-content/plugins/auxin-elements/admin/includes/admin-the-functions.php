...
function auxin_count_bundled_plugins_have_update(){
    // Check transient
    if (  false === ( $tgmpa_counter = get_transient( 'auxin_count_bundled_plugins_have_update' ) ) ) {
        // Get instance
        ...
        foreach ( $tgmpa_instance->plugins as $slug => $plugin ) {
            if (  $plugin['source_type'] === 'bundled' and $tgmpa_instance->does_plugin_have_update( $slug ) ) {
                $tgmpa_counter++;
            }
        }
        set_transient( 'auxin_count_bundled_plugins_have_update', $tgmpa_counter, 12 * HOUR_IN_SECONDS );
    }
