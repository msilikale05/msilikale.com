...
class Auxin_Demo_Importer {
                ...
                foreach ( $menu_data['items'] as $item_key => $item_value ) {
                    ...
                    switch ( $item_value['menu-item-type'] ) {
                        ...
                        case 'taxonomy':
                            ...
                            break;
                        default:
                            if( strpos( $meta_value, '{{demo_home_url}}' ) ) {
    ...
    public function update_elementor_data( $meta ) {
        ...
        preg_match_all('/\{\s*"image"\s*:\s*(.+?)\s*\}/', $meta, $images, PREG_SET_ORDER );
        preg_match_all('/\s*"poster"\s*:\s*(.+?)\s*\}/', $meta, $poster, PREG_SET_ORDER );
        preg_match_all('/\{\s*"media"\s*:\s*(.+?)\s*\}\}/', $meta, $media, PREG_SET_ORDER );
        preg_match_all('/\{\s*"src"\s*:\s*(.+?)\s*\}\}/', $meta, $src, PREG_SET_ORDER );
        preg_match_all('/\{\s*"wp_gallery"\s*:\s*(.+?)\s*\}\]/', $meta, $wp_gallery, PREG_SET_ORDER );

        $matches = array_merge( $images, $media, $src, $poster);
